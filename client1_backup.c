...
*/
static int dump_tcp_conn_state(int fd,struct libsoccr_sk_data* data)
{
    ...
    struct soccr_tcp_info ti;
    refresh_sk(fd,data,&ti);

    if (get_queue(fd, TCP_RECV_QUEUE, &data->inq_seq, data->inq_len))
		return -5;

	if (get_queue(fd, TCP_SEND_QUEUE, &data->outq_seq, data->outq_len))
		return -6;
}
